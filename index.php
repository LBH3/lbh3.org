<?php require "php/web_constants.php"; ?>
<!DOCTYPE HTML>
<html>
	<?php include 'includes/html_head.php'; ?>
  <body>
    <header>
      <?php include 'includes/header.html'; ?>
      <?php include 'includes/top_nav.php'; ?>
    </header>
    <main class="container home">
      <p>
        <small>
          The Long Beach kennel was founded over 30 years ago.
          We run Thursday evening during Spring/Summer &amp; Sunday morning in the Fall/Winter.
          We often have an attendance of 75+.
          Visitors &amp; Virgins are always welcome!
          Your run donation covers Hare essentials, pre– and post–run beer/soda/munchies.
          Please bring a vessel for your beverage.
          We hope to see you at LBH3.
          On On!!!
        </small>
      </p>

			<hr>

      <h2>Run #1797</h2>
      <dl class="row">
        <dt class="col-sm-3 text-sm-right">Date:</dt>
        <dd class="col-sm-9"><strong>Thursday</strong> 3/16 ~ 6:30 p.m.</dd>
        <dt class="col-sm-3 text-sm-right">Start:</dt>
        <dd class="col-sm-9">11155 183rd St, Cerritos, CA 90703</dd>
        <dt class="col-sm-3 text-sm-right">Hares:</dt>
        <dd class="col-sm-9">Kowpaddy Kowboy &amp; Spitty Spitty Gang Bang</dd>
        <dt class="col-sm-3 text-sm-right">Donation:</dt>
        <dd class="col-sm-9">$5 for run</dd>
      </dl>

			<iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3312.8610962105263!2d-118.09828478478974!3d33.86747048065664!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x80dd2d71c6ee5375%3A0x1655f200f2611ea3!2s11155+183rd+St%2C+Cerritos%2C+CA+90703!5e0!3m2!1sen!2sus!4v1489466463859" width="100%" height="368" frameborder="0" style="border:0" allowfullscreen></iframe>

      <dl class="row">
        <dt class="col-md-3 text-md-right">From the hares:</dt>
        <dd class="col-md-9">
					<p>
						The first Thursday run of the year will be in Cerritos. Come on out before you leave for Hash Springs and pre-lube at the St. KowPaddy night run with Spitty Spitty Gang Bang as the kohare! A couple beer checks and some urban shiggy. It should be fun.
          </p>
        </dd>
        <dt class="col-md-3 text-md-right">Directions:</dt>
        <dd class="col-md-9">
					<p>
						Very simple. From wherever you are, go to the Cerritos Mall. At 183rd and Gridley, drive to the back of the Guitar Center area. LFH.
          </p>
        </dd>
      </dl>
    </main>
    <footer>
      <p class="container text-muted">
        For website issues, <a href="mailto:<?php echo wbm_email;?>">email <?php echo wbm_name;?></a>
      </p>
    </footer>
  </body>
</html>
