...
Promise.allSettled(promises).then((data) => {
  ...
  Promise.allSettled(recurringEvents).then((data) => {
    ...
    calendarDatesWithMonths.map(({dates}) => {
      ...
      return `
                              <a class="d-block mb-4" data-bs-target="#event-${event.id}" data-bs-toggle="modal" style="border-top: ${event[computedSymbol].startDay === event[computedSymbol].endDay ? '.25rem' : '.5rem'} solid ${event[computedSymbol].calendarData.color}">
